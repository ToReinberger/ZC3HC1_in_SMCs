...
def get_network_interactions() -> 'Getting the STRING network interactions':
    ...
    cluster_colors_temp = ["gray", "darkorange", "slateblue", "green", "#0066CC", "firebrick",
                           "darkcyan", "plum", "peru", "firebrick", "lightgreen", "c", "orchid",
                           "royalblue"]
    ...
    for gene in list(G.nodes):
        gene_in_cluster = False
        if gene in my_genes and not gene == "CCNB1":
            print(gene)
            foldchange = file.loc[file.gene == gene, "log2FoldChange"].values[0]
            color_values.append(foldchange)
            for i in range(1, num_cluster):
                if gene in cluster_dict[i]:
                    gene_in_cluster = True
                    break
            if gene == "ZC3HC1":  # or abs(foldchange) > 1.19:
    ...
    # color_values = file.loc[file.gene.isin(list(G.nodes)), "log2FoldChange"]
    ...
    """
    for gene_edges in list(G.edges):
        if (gene_edges[0] in my_genes and gene_edges[1] in connection) or \
                (gene_edges[1] in my_genes and gene_edges[0] in connection) or \
                (gene_edges[1] in connection and gene_edges[0] in connection) or \
                (gene_edges[1] in my_genes and gene_edges[0] in my_genes):
            edge_colors.append("orange")
            edge_width.append(30)
        else:
    ...
    if plot_patches:
        ...
        for i in range(2, num_cluster + 1):
            ...
            ycenter = cluster_pos[i][1] + 0.004
            width = cluster_pos[i][2] + 0.03
            height = cluster_pos[i][3] + 0.06
            angle = 0
    ...
    # plt.bar(height=enrichment_out_sum["p_value"], y=enrichment_out_sum["GO process"],
    #        color=enrichment_out_sum["cluster_color"])
    from matplotlib import colors
    # cluster_colors_temp2 = enrichment_out_sum["cluster_color"].values
    # cluster_colors_temp2[:] = [colors.to_rgba(x) for x in cluster_colors_temp2]
    foldchange_values = []
    go_terms = enrichment_out_sum["GO process"].values
    go_terms[:] = [x[0].capitalize() + x[1:] for x in go_terms]
    plt.barh(y=go_terms,
             height=0.8,
    ...
    my_palette = dict(zip(list(np.arange(0, num_cluster)), cluster_colors_temp[1: num_cluster]))
    sns.violinplot(data=cluster_foldchange[1:], palette=my_palette, orient="horizontal",
                   alpha=0.6, width=1.3, inner=None, linewidth=0.0
                   )
    ...
    plt.subplots_adjust(right=0.95, left=0.05, wspace=0.05, hspace=0.05, bottom=0.05, top=0.95)
    c1, c2 = 0, 0
    dim = int(num_cluster/2)
    dim2 = 2
    show_subcluster = True
    if show_subcluster:
        print('Plot subcluster')
        counter4 = 1
        for subcluster in cluster_dict:
            if subcluster > 0:
                if c1 == dim:
                    c1 = 0
                    c2 += 1
                plt.subplot2grid((dim2, dim), (c2, c1))
                c1 += 1
                sub_network = G.subgraph(cluster_dict[subcluster])
                positions = nx.kamada_kawai_layout(sub_network)
                nx.draw(sub_network,
                        with_labels=True,
                        # labels=dict(zip(list(G.nodes), relabels)),
                        pos=positions,  # default: kamada_kawai_layout
                        # width=edge_width,
                        edge_color=cluster_colors_temp[counter4],
                        node_color=cluster_colors_temp[counter4],
                        node_size=400,
                        alpha=0.7,
                        # fontdict={"size": 1, "weight": "bold"},
                        font_weight="bold",
                        font_size=9.5)
                print(len(list(sub_network.nodes())))
                print(subcluster)
                counter4 += 1
    plt.savefig("subcluster.pdf")
...
if __name__ == '__main__':
    ...
    # file = file[file["log2FoldChange"] < -0.5]
    file = file[abs(file["log2FoldChange"]) > 0.4]
    file_under = file[file["log2FoldChange"] < -0.6]
    file_over = file[file["log2FoldChange"] > 0.6]
    ...
    gene_regulation = []
    my_genes_temp = my_genes
    my_genes_temp.append("CCNB1")

    params = {
        "identifiers":  "%0d".join(my_genes_temp),  # your proteins
        "species": 9606,  # species NCBI identifier = Homo sapiens
